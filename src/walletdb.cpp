...
CWalletDB::ReorderTransactions(CWallet* pwallet)
{
    ...
    //for (boost::unordered_map<uint256, CWalletTx>::iterator it = pwallet->mapWallet.begin(); it != pwallet->mapWallet.end(); ++it)
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& it, pwallet->mapWallet)
    {
        CWalletTx* wtx = &(it.second); //&((*it).second);
...
void ThreadFlushWalletDB(void* parg)
{
    ...
    RenameThread("bitexchange-wallet");
