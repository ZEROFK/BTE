...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new BitExchange address for receiving payments.  "
    ...
    if (account.vchPubKey.IsValid())
    {
        ...
        //for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
           //  it != pwalletMain->mapWallet.end() && account.vchPubKey.IsValid();
            // ++it)
        BOOST_FOREACH(const PAIRTYPE(uint256, CWalletTx)& it, pwalletMain->mapWallet)
        {
            const CWalletTx& wtx = it.second; //(*it).second;
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current BitExchange address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <bitexchangeaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitExchange address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <bitexchangeaddress> <amount> [comment] [comment-to] [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <bitexchangeaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <bitexchangeaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <bitexchangeaddress> [minconf=1]\n"
            "Returns the total amount received by <bitexchangeaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tobitexchangeaddress> <amount> [minconf=1] [comment] [comment-to] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    Array retHexHashes; // to return transaction hashes if stealth/multiples
    bool bHasStealthTx = false;
	...
	if(IsStealthAddress(s.name_))
	{
	    ...
	    retHexHashes.push_back(sendtostealthaddress(sRet, false));
	}
	else if(!bHasStealthTx)
	{
            CBitcoinAddress address(s.name_);
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid BitExchange address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a BitExchange address or hex-encoded public key\n"
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("bitexchange-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("bitexchange-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; BitExchange server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <bitexchangeaddress>\n"
            "Return information about <bitexchangeaddress>.");
	...
	isValid = IsStealthAddress(params[0].get_str());
	    ...
	    {
		throw runtime_error("GenerateRandomSecret failed.");
		return false;
	    };
    
	    if (StealthSecret(ephem_secret, sxAddress.scan_pubkey, sxAddress.spend_pubkey, secretShared, pkSendTo) != 0)
	    {
		throw runtime_error("Could not generate receiving public key.");
		return false;
	    };
	    ...
	    if (!cpkTo.IsValid())
	    {
		throw runtime_error("Invalid public key generated.");
		return false;
	    };
	    ...
	    {
		throw runtime_error("Could not generate ephem public key.");
		return false;
	    };
    
		    if (fDebug)
		    {
			printf("validateaddress generated pubkey %"PRIszu": %s\n", pkSendTo.size(), HexStr(pkSendTo).c_str());
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <bitexchangepubkey>\n"
            "Return information about <bitexchangepubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new BitExchange stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid BitExchange stealth address."));
        return result;
    };
