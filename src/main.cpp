...
unsigned int nStakeMinAge = (60 * 10); // 10 minutes
unsigned int nStakeMaxAge = -1; // NO MAX
...
const string strMessageMagic = "BitExchange Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
        nSubsidy = 21000000 * COIN; // coins
    }
        else if(pindexBest->nHeight < START_NET)
    {
        nSubsidy = 0 * COIN; // Keep network staking
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > START_NET)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitExchange", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block        
        const char* pszTimestamp = "BitExchange Genesis Creation 7/9";
        CTransaction txNew;
        txNew.nTime = 1468107402;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1468107402;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1907099 : 0;
        
        
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc3f2423c140a100d50e10a150415b026d6b5f9afbd597fa6c49cdb9122bdbe22"));
        block.print();
...
void PrintBlockTree()
{
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CBlockIndex*)& mi, mapBlockIndex)
    {
        CBlockIndex* pindex = mi.second;
...
unsigned char pchMessageStart[4] = { 0x2f, 0xd9, 0xe2, 0x20 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "getdata")
    {
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                {
                    LOCK(cs_mapRelay);
                    boost::unordered_map<CInv, CDataStream>::iterator mi = mapRelay.find(inv);
                    if (mi != mapRelay.end()) {
    ...
    else if (strCommand == "reply")
    {
        ...
        CRequestTracker tracker;
        {
            ...
            boost::unordered_map<uint256, CRequestTracker>::iterator mi = pfrom->mapRequests.find(hashReply);
